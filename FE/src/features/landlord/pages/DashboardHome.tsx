...
import InsufficientBalanceModal from '@/features/landlord/components/InsufficientBalanceModal';
import { useOutletContext } from '@/lib/navigation/router-compat';
import VideoRequestModal from '@/features/landlord/components/VideoRequestModal';
import HotListingRequestModal from '@/features/landlord/components/HotListingRequestModal';
import type { EntityId, ListingSummary, User } from '@/types';

interface DashboardListing extends ListingSummary {
    name?: string;
...
export default function DashboardHome() {
    ...
    const handleDelete = async (id: EntityId): Promise<void> => {
        ...
        try {
            ...
            const message = axios.isAxiosError<{ message?: string }>(error)
                ...
                : undefined;
            alert(message || 'Có lỗi xảy ra khi xóa tin đăng!');
        }
    };

    const handleHideListing = async (id: EntityId): Promise<void> => {
        if (
            !confirm(
        ...
        try {
            ...
            const message = axios.isAxiosError<{ message?: string }>(error)
                ? error.response?.data?.message
                : undefined;
            alert(message || 'Có lỗi xảy ra khi ẩn tin đăng!');
        }
    };

    const handleUnhideListing = async (id: EntityId): Promise<void> => {
    ...
    const formatPrice = (price?: number | string): string => {
        ...
        if (!numericPrice) return '0 đồng';
        if (numericPrice >= 1000000) {
            return `${(numericPrice / 1000000).toFixed(1)} triệu/tháng`;
        }
        return `${numericPrice.toLocaleString('vi-VN')} đồng/tháng`;
    };
    ...
    const handleTypeSelect = (type: (typeof rentalTypes)[number]): void => {
        if (type.requiresFee) {
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        {listing.status === 'published' && listing.is_hot === 1 && listing.hot_until && (
                                            <>
                                                ...
                                                {new Date(listing.hot_until).getTime() - new Date().getTime() < 3 * 24 * 60 * 60 * 1000 && (
                                                    <button
                                                        onClick={() => {
                                                            setSelectedListing(listing);
                                                            setShowHotModal(true);
