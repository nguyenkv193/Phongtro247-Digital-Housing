...
import React, { useState, useRef, useEffect, type ChangeEvent, type FormEvent, type MouseEvent } from 'react';
import axios from 'axios';
import type { EntityId, ListingLocation, Tenant } from '@/types';

interface ManagedTenant extends Tenant {
    birthday?: string;
    gender?: string;
    cccd?: string;
    stay_status?: string;
...
const TenantManagement = () => {
    ...
    const [wards, setWards] = useState<ListingLocation[]>([]);
    const [tenants, setTenants] = useState<ManagedTenant[]>([]);
    const [filterStatus, setFilterStatus] = useState('Tất cả');

    const owner_id = typeof window !== 'undefined' ? localStorage.getItem('user_id') || 1 : 1;
    ...
    const handleChange = (
        ...
        setFormData(prev => ({ ...prev, [name as keyof TenantFormData]: value }));
    };
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            const message = axios.isAxiosError<{ message?: string }>(err)
                ...
                : undefined;
            alert(message || 'Lỗi khi thêm khách thuê');
        }
    };
