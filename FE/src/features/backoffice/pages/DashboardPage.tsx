...
import { EmptyState, LoadingState, StatusBadge } from '@/features/backoffice/components/BackofficeUi';
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button type="button" onClick={() => void load(true)} disabled={refreshing} className="inline-flex h-9 cursor-pointer items-center justify-center gap-2 self-start rounded-md border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60 sm:self-auto">
                        ...
                        {recentListings.length ? recentListings.map(listing => (
                            <div key={listing.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                                ...
                                <div className="min-w-0 flex-1">
                                    <Link href={`/listing/${listing.id}`} target="_blank" rel="noreferrer" className="block cursor-pointer truncate text-sm font-medium text-slate-800 hover:text-blue-700">{listing.name}</Link>
