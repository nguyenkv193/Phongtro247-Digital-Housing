package com.phongtro247.housing.modules.transactions.service;
...
import com.phongtro247.housing.modules.transactions.dto.TransactionResponse;
...
public class TransactionService {
    ...
    private String formatStatus(TransactionEntity transaction, String description) {
        ...
        if (normalized.contains(MessageCatalog.TRANSACTION_SUCCESS_STATUS.toLowerCase(Locale.ROOT))
                || normalized.contains("success")) {
            return MessageCatalog.TRANSACTION_SUCCESS_STATUS;
        }
        if (normalized.contains(MessageCatalog.TRANSACTION_FAILED_STATUS.toLowerCase(Locale.ROOT))
                || normalized.contains("failed")) {
            return MessageCatalog.TRANSACTION_FAILED_STATUS;
        }
        if (normalized.contains("pending")
                || normalized.contains(MessageCatalog.TRANSACTION_PROCESSING_STATUS.toLowerCase(Locale.ROOT))) {
            return MessageCatalog.TRANSACTION_PROCESSING_STATUS;
        }
        return MessageCatalog.TRANSACTION_SUCCESS_STATUS;
    }
